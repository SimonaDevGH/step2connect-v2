...
const CLIENT_ID = process.env.COGNITO_USER_POOL_CLIENT_ID || process.env.VITE_COGNITO_USER_POOL_CLIENT_ID;
...
async function requireAuth(req, res, next) {
  if (!POOL_ID || !CLIENT_ID) {
    return res.status(500).json({ error: 'Cognito configuration is incomplete' });
  }
